...
public final class Miniball2 {
	...
	public static Circle2 makeCircle(List<Point2> points) {
		// Clone list to preserve the caller's data, randomize order
		List<Point2> shuffled = new ArrayList<>(points);
		Collections.shuffle(shuffled, new Random());
		
		// Progressively add points to circle or recompute circle
		Circle2 c = null;
		for (int i = 0; i < shuffled.size(); i++) {
			Point2 p = shuffled.get(i);
	...
	private static Circle2 makeCircleOnePoint(List<Point2> points, Point2 p) {
	...
	private static Circle2 makeCircleTwoPoints(List<Point2> points, Point2 p, Point2 q) {
		Circle2 circ = makeDiameter(p, q);
		Circle2 left  = null;
		Circle2 right = null;
		
		// For each point not in the two-point circle
		Point2 pq = q.subtract(p);
		for (Point2 r : points) {
			if (circ.contains(r))
				continue;
	...
	static Circle2 makeDiameter(Point2 a, Point2 b) {
		Point2 c = new Point2((a.x + b.x) / 2, (a.y + b.y) / 2);
		return new Circle2(c, Math.max(c.distance(a), c.distance(b)));
	}
	
	
	static Circle2 makeCircumcircle(Point2 a, Point2 b, Point2 c) {
